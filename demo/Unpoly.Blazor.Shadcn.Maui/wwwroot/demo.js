...
// ---- command palette closes on navigate ------------------------------------------------------------------
// In the web head a CommandItem carries up-target="body", so Unpoly swaps the page and the
// palette's <dialog> never lingers. The MAUI head has no Unpoly nav — Blazor's router owns the
// link, the page swaps, and the dialog would stay open over it. Close it on activation.
up.compiler('command-dialog[data-slot="command-dialog"]', (dialog) => {
  const onItemClick = (event) => {
    const item = event.target.closest('[data-slot="command-item"] a')
    if (item) dialog.close()
  }
  dialog.addEventListener('click', onItemClick)
  // Also on a keyboard Enter/activation of the focused item.
  const onKeydown = (event) => {
    if (event.key === 'Enter' || event.key === ' ') {
      const active = dialog.querySelector('[data-slot="command-item"][data-selected="true"] a')
      if (active) dialog.close()
    }
  }
  dialog.addEventListener('keydown', onKeydown)
  return () => {
    dialog.removeEventListener('click', onItemClick)
    dialog.removeEventListener('keydown', onKeydown)
  }
})

// ---- sample POST helper ----------------------------------------------------------------
// The web head answers a sample's POST from a server and Unpoly swaps the reply into the
// #preview-... slot. A WebView has no server, so a sample posts to THIS helper instead: it
// writes a "reply" into the slot the sample already named. Same id, same contract, no server.
window.demoSample = {
  post(previewId, form) {
    const slot = document.getElementById(previewId)
    if (!slot) return
    const fields = form ? [...new FormData(form).entries()]
      .filter(([k]) => k !== '__RequestVerificationToken')
      .map(([k, v]) => `${k}=${v}`).join(', ') : 'no fields'
    slot.innerHTML =
      '<div class="mt-2 rounded-md border border-dashed px-3 py-2 text-sm text-muted-foreground">' +
      'POST — ' + (fields || 'no fields') + '</div>'
  },
  answer(previewId, html) {
    const slot = document.getElementById(previewId)
    if (slot) slot.innerHTML = html
  },
}

// ---- sample form interceptor ------------------------------------------------------------
// Every sample <form up-...> in the web head POSTs/GETs to the server and Unpoly swaps the reply
// into the target (or navigates). A WebView has no server and no Unpoly round trip, so this
// intercepts a sample submit, prevents the native navigation (which would 404), and writes the
// reply into the target slot the form names. The reply is what the web server would have said:
// the path and the posted fields. One interceptor, every sample — no per-form C# needed.
function interceptFormSubmit(form, event) {
  event.preventDefault()
  const target = (form.getAttribute('up-target') || '').trim().replace(/^#/, '')
  const fields = form
    ? [...new FormData(form).entries()].filter(([k]) => k !== '__RequestVerificationToken')
        .map(([k, v]) => `${k}=${v}`).join(', ')
    : 'no fields'
  const reply = 'POST — ' + (fields || 'no fields')

  if (window.demoSample && target && target !== 'body') {
    // A #preview-... slot or a named element: write the reply in place.
    window.demoSample.post('#' + target, form)
  } else if (target === 'body') {
    // up-target="body" is the whole shell (an auth boundary). In a WebView there is no server
    // to transition, so the reply is appended after the form — the reader sees the round trip
    // without the page being replaced.
    const result = document.createElement('div')
    result.className = 'mt-2 rounded-md border border-dashed px-3 py-2 text-sm text-muted-foreground'
    result.textContent = reply
    form.closest('.preview')?.appendChild(result) ?? form.after(result)
  } else {
    // A GET form without a named target (calendar, table) was a navigation. Keep it visible.
    const slot = document.getElementById(target)
    if (slot) slot.innerHTML = reply
  }
}
up.compiler('form[up-target], form[up-autosubmit], form[method="post"], form[method="get"]', (form) => {
  const onsubmit = (event) => interceptFormSubmit(form, event)
  form.addEventListener('submit', onsubmit)
  // up-autosubmit fires on change of a control inside the form, not on submit.
  const onchange = (event) => {
    if (form.hasAttribute('up-autosubmit') && event.target.closest('select,input,textarea')) {
      form.requestSubmit()
    }
  }
